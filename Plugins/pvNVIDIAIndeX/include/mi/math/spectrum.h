...
inline Spectrum clamp(const Spectrum& c, const Spectrum& low, const Spectrum& high)
{
  ...
  return Spectrum(
    clamp(c[0], low[0], high[0]), clamp(c[1], low[1], high[1]), clamp(c[2], low[2], high[2]));
}
...
inline Spectrum clamp(const Spectrum& c, const Spectrum& low, Float32 high)
{
  mi_math_assert_msg(c.size() == 3, "precondition");
  mi_math_assert_msg(low.size() == 3, "precondition");
  return Spectrum(clamp(c[0], low[0], high), clamp(c[1], low[1], high), clamp(c[2], low[2], high));
}
...
inline Spectrum clamp(const Spectrum& c, Float32 low, const Spectrum& high)
{
  mi_math_assert_msg(c.size() == 3, "precondition");
  mi_math_assert_msg(high.size() == 3, "precondition");
  return Spectrum(clamp(c[0], low, high[0]), clamp(c[1], low, high[1]), clamp(c[2], low, high[2]));
}
...
{
  mi_math_assert_msg(c.size() == 3, "precondition");
  return Spectrum(clamp(c[0], low, high), clamp(c[1], low, high), clamp(c[2], low, high));
}
...
inline Spectrum gamma_correction(const Spectrum& spectrum, ///< spectrum to be corrected
  Float32 gamma_factor) ///< gamma factor, must be greater than zero.
{
  ...
  return Spectrum(fast_pow(spectrum[0], f), fast_pow(spectrum[1], f), fast_pow(spectrum[2], f));
}

/// Compares the two given values elementwise for equality within the given epsilon.
inline bool is_approx_equal(const Spectrum& lhs, const Spectrum& rhs, Float32 e)
{
  mi_math_assert_msg(lhs.size() == 3, "precondition");
  mi_math_assert_msg(rhs.size() == 3, "precondition");
  return is_approx_equal(lhs[0], rhs[0], e) && is_approx_equal(lhs[1], rhs[1], e) &&
    is_approx_equal(lhs[2], rhs[2], e);
}

/// Returns the elementwise linear interpolation between \p c1 and \c c2, i.e., it returns
/// <tt>(1-t) * c1 + t * c2</tt>.
inline Spectrum lerp(const Spectrum& c1, ///< one spectrum
  ...
  const Spectrum& t)                     ///< interpolation parameter in [0,1]
{
  ...
  return Spectrum(lerp(c1[0], c2[0], t[0]), lerp(c1[1], c2[1], t[1]), lerp(c1[2], c2[2], t[2]));
}
  ...
  Float32 t)                             ///< interpolation parameter in [0,1]
{
  ...
  return Spectrum(lerp(c1[0], c2[0], t), lerp(c1[1], c2[1], t), lerp(c1[2], c2[2], t));
}
...
{
  mi_math_assert_msg(c.size() == 3, "precondition");
  return Spectrum(log2 MI_PREVENT_MACRO_EXPAND(c[0]), log2 MI_PREVENT_MACRO_EXPAND(c[1]),
...
inline Spectrum smoothstep(const Spectrum& a, const Spectrum& b, const Spectrum& c)
{
  ...
  mi_math_assert_msg(c.size() == 3, "precondition");
  return Spectrum(
    smoothstep(a[0], b[0], c[0]), smoothstep(a[1], b[1], c[1]), smoothstep(a[2], b[2], c[2]));
}
...
inline Spectrum smoothstep(const Spectrum& a, const Spectrum& b, Float32 x)
{
  mi_math_assert_msg(a.size() == 3, "precondition");
  mi_math_assert_msg(b.size() == 3, "precondition");
  return Spectrum(smoothstep(a[0], b[0], x), smoothstep(a[1], b[1], x), smoothstep(a[2], b[2], x));
}
...
{
  mi_math_assert_msg(c.size() == 3, "precondition");
  return isfinite MI_PREVENT_MACRO_EXPAND(c[0]) && isfinite MI_PREVENT_MACRO_EXPAND(c[1]) &&
...
{
  mi_math_assert_msg(c.size() == 3, "precondition");
  return isinfinite MI_PREVENT_MACRO_EXPAND(c[0]) || isinfinite MI_PREVENT_MACRO_EXPAND(c[1]) ||
...
{
  mi_math_assert_msg(c.size() == 3, "precondition");
  return isnan MI_PREVENT_MACRO_EXPAND(c[0]) || isnan MI_PREVENT_MACRO_EXPAND(c[1]) ||
