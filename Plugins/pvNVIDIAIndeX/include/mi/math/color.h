...
public:
  ...
  */
  ...
  inline bool is_black() const { return (r == 0.0f) && (g == 0.0f) && (b == 0.0f); }

  /// Returns the intensity of the RGB components, equally weighted.
  inline Float32 linear_intensity() const { return (r + g + b) * Float32(1.0 / 3.0); }
  ...
  inline Float32 ntsc_intensity() const { return r * 0.299f + g * 0.587f + b * 0.114f; }

  /// Returns the intensity of the RGB components, weighted according to the CIE standard.
  ///
  /// Components are weighted to match the subjective color brightness perceived by the human eye;
  /// green appears brighter than blue.
  inline Float32 cie_intensity() const { return r * 0.212671f + g * 0.715160f + b * 0.072169f; }
...
{
  return Color(clamp(c.r, low.r, high.r), clamp(c.g, low.g, high.g), clamp(c.b, low.b, high.b),
...
{
  return Color(clamp(c.r, low.r, high), clamp(c.g, low.g, high), clamp(c.b, low.b, high),
...
{
  return Color(clamp(c.r, low, high.r), clamp(c.g, low, high.g), clamp(c.b, low, high.b),
...
{
  return Color(
    clamp(c.r, low, high), clamp(c.g, low, high), clamp(c.b, low, high), clamp(c.a, low, high));
}
...
inline Color gamma_correction(const Color& color, ///< color to be corrected
  Float32 gamma_factor)                           ///< gamma factor, must be greater than zero.
{
  ...
  return Color(fast_pow(color.r, f), fast_pow(color.g, f), fast_pow(color.b, f), color.a);
}

/// Compares the two given values elementwise for equality within the given epsilon.
inline bool is_approx_equal(const Color& lhs, const Color& rhs, Float32 e)
{
  return is_approx_equal(lhs.r, rhs.r, e) && is_approx_equal(lhs.g, rhs.g, e) &&
    is_approx_equal(lhs.b, rhs.b, e) && is_approx_equal(lhs.a, rhs.a, e);
}

/// Returns the elementwise linear interpolation between \p c1 and \c c2, i.e., it returns
/// <tt>(1-t) * c1 + t * c2</tt>.
inline Color lerp(const Color& c1, ///< one color
  const Color& c2,                 ///< second color
  const Color& t)                  ///< interpolation parameter in [0,1]
{
  return Color(
    lerp(c1.r, c2.r, t.r), lerp(c1.g, c2.g, t.g), lerp(c1.b, c2.b, t.b), lerp(c1.a, c2.a, t.a));
}
...
{
  return Color(log2 MI_PREVENT_MACRO_EXPAND(c.r), log2 MI_PREVENT_MACRO_EXPAND(c.g),
    log2 MI_PREVENT_MACRO_EXPAND(c.b), log2 MI_PREVENT_MACRO_EXPAND(c.a));
}
...
{
  return Color(smoothstep(a.r, b.r, c.r), smoothstep(a.g, b.g, c.g), smoothstep(a.b, b.b, c.b),
...
{
  return Color(smoothstep(a.r, b.r, x), smoothstep(a.g, b.g, x), smoothstep(a.b, b.b, x),
...
{
  return isfinite MI_PREVENT_MACRO_EXPAND(c.r) && isfinite MI_PREVENT_MACRO_EXPAND(c.g) &&
    isfinite MI_PREVENT_MACRO_EXPAND(c.b) && isfinite MI_PREVENT_MACRO_EXPAND(c.a);
}

/// Indicates whether any component of the color is infinite.
inline bool isinfinite MI_PREVENT_MACRO_EXPAND(const Color& c)
{
  return isinfinite MI_PREVENT_MACRO_EXPAND(c.r) || isinfinite MI_PREVENT_MACRO_EXPAND(c.g) ||
    isinfinite MI_PREVENT_MACRO_EXPAND(c.b) || isinfinite MI_PREVENT_MACRO_EXPAND(c.a);
}

/// Indicates whether any component of the color is "not a number".
inline bool isnan MI_PREVENT_MACRO_EXPAND(const Color& c)
{
  return isnan MI_PREVENT_MACRO_EXPAND(c.r) || isnan MI_PREVENT_MACRO_EXPAND(c.g) ||
    isnan MI_PREVENT_MACRO_EXPAND(c.b) || isnan MI_PREVENT_MACRO_EXPAND(c.a);
}
...
inline Color Color::clip(Clip_mode mode, bool desaturate) const
{
  ...
  return Color(math::clamp(col.r, 0.0f, max_val), math::clamp(col.g, 0.0f, max_val),
    math::clamp(col.b, 0.0f, max_val), col.a);
}
...
inline Color Color::desaturate(Float32 maxval) const
{
  ...
  if (clip)
  {
    ...
    return Color(math::clamp(axis + drds * sat, 0.0f, maxval),
      math::clamp(axis + dgds * sat, 0.0f, maxval), math::clamp(axis + dbds * sat, 0.0f, maxval),
      a);
  }
