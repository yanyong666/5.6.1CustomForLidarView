...
vtkMFIXReader::vtkMFIXReader()
{
  ...
  this->GetExecutive()->SetOutputData(2, grid);
  this->GetExecutive()->SetOutputData(3, grid);
  grid->Delete();
...
vtkMFIXReader::~vtkMFIXReader()
{
  ...
  this->InletMesh->Delete();
  this->OutletMesh->Delete();
  this->ObstructionMesh->Delete();
  ...
  vtkInformationVector *outputVector)
{
  ...
  vtkUnstructuredGrid *output1 = vtkUnstructuredGrid::SafeDownCast(
    outInfo1->Get(vtkDataObject::DATA_OBJECT()));
  vtkUnstructuredGrid *output2 = vtkUnstructuredGrid::SafeDownCast(
    outInfo2->Get(vtkDataObject::DATA_OBJECT()));
  vtkUnstructuredGrid *output3 = vtkUnstructuredGrid::SafeDownCast(
    outInfo3->Get(vtkDataObject::DATA_OBJECT()));
  vtkUnstructuredGrid *output4 = vtkUnstructuredGrid::SafeDownCast(
    outInfo4->Get(vtkDataObject::DATA_OBJECT()));
  vtkDebugMacro( << "Reading MFIX file");

  this->MakeMesh(output1, output2, output3, output4);
  return 1;
}
...
//----------------------------------------------------------------------------
void vtkMFIXReader::AddCellToMesh(vtkCell *aCell, const int val)
{
  // val represents a value in this->Flag
  if ( val < 10 )
    {
...
//----------------------------------------------------------------------------
void vtkMFIXReader::MakeMesh(vtkUnstructuredGrid *output1,
                             vtkUnstructuredGrid *output2,
                             vtkUnstructuredGrid *output3,
                             vtkUnstructuredGrid *output4)
{
  output1->Allocate();
  output2->Allocate();
  output3->Allocate();
    ...
    if (!strcmp(this->CoordinateSystem,"CYLINDRICAL") &&
      ...
      for (int j = 0; j < this->JMaximum2; j++)
        ...
        for (int i = 0; i < this->IMaximum2; i++)
          ...
          this->AddCellToMesh(this->AQuad, this->Flag->GetValue(count));
        ...
        for (int j = 0; j < this->JMaximum2; j++)
          ...
          for (int i = 0; i < this->IMaximum2; i++)
            ...
            if (( k == (this->KMaximum2-2)) && (i != 1))
              ...
              this->AddCellToMesh(this->AHexahedron, 
                this->Flag->GetValue(count));
              }
              ...
              this->AddCellToMesh(this->AWedge, 
                this->Flag->GetValue(count));
              }
            else if (( k == (this->KMaximum2-2)) && (i == 1))
              {
  ...
  // If mesh has already been made copy it to output
  output1->DeepCopy(this->FluidMesh);  
  output2->DeepCopy(this->InletMesh);  
  output3->DeepCopy(this->OutletMesh);  
  output4->DeepCopy(this->ObstructionMesh);  
  ...
  for (int j = 0; j <= this->VariableNames->GetMaxId(); j++)
    ...
    if ( this->CellDataArraySelection->GetArraySetting(j) == 1 )
      ...
      if (first == 0)
        {
        output1->GetCellData()->SetScalars(this->CellDataArray[j]);
        }
      else
        {
        output1->GetCellData()->AddArray(this->CellDataArray[j]);
        }
      double tempRange[2];
...
const int vtkMFIXReader::GetCellArrayNumComponents(int index)
{
  return this->VariableComponents->GetValue(index); 
}
...
int vtkMFIXReader::FillOutputPortInformation(int port, vtkInformation* info)
{
