...
///
/// The interface class allows an application-writer to efficiently manipulate the scene
/// configuration
/// and set up and reduces the efforts to tinker with the hierarchical scene description.
/// Nonetheless, defining the scene description explicitly and managing the scene in the application
/// logic
/// represents the method of choice.
///
/// In addition, the interface class provides access to entities in the scene that otherwise are
/// hidden
/// from the application-writer, such as a default light source defined by NVIDIA IndeX if no light
/// source
...
///
class IScene_convenience_manipulation : public mi::base::Interface_declare<0xc8934dfa, 0xc049,
                                          0x401e, 0x90, 0x3a, 0x8d, 0x18, 0x2f, 0xfa, 0x39, 0x81>
{
public:
  ...
  ///                                 internally.
  ///
  virtual void set_heightfield_intersection_highlighting(bool enable, mi::neuraylib::Tag_struct tag,
    mi::neuraylib::IDice_transaction* dice_transaction) = 0;

  /// Enabling and disabling the highlighting of the intersections of all triangle meshes with a
  /// plane.
  ...
  ///                             internally.
  ///
  virtual void set_trianglemesh_intersection_highlighting(bool enable,
    mi::neuraylib::Tag_struct tag, mi::neuraylib::IDice_transaction* dice_transaction) = 0;
  ...
  ///                                 local space.
  /// \param[in]  smoothness          The amount of smoothing applied to the intersection
  /// highlighting.
  ...
  ///                                 internally.
  ///
  virtual void set_default_intersection_highlighting_properties(const mi::math::Color_struct& color,
    mi::Float32 width, mi::Float32 smoothness, mi::Float32 discontinuity_limit,
    mi::neuraylib::IDice_transaction* dice_transaction) = 0;
  ...
  ///                             internally.
  ///
  virtual void set_values_of_default_light(const mi::math::Color_struct& intensity,
  ...
  ///                             internally.
  ///
  virtual void get_values_of_default_light(mi::math::Color_struct& intensity,
