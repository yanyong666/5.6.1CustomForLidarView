...
public:
  ...
  */
  ...
  inline Matrix(Transposed_copy_tag, const Matrix<T, COL, ROW>& other)
  {
  ...
  /// type.
  template <typename T2>
  inline Matrix(Transposed_copy_tag, const Matrix<T2, COL, ROW>& other)
  {
    for (Size i(0u); i < ROW; ++i)
      for (Size j(0u); j < COL; ++j)
  ...
  ///
  /// \pre <tt>ROW == 1</tt>
  inline explicit Matrix(const Row_vector& v0)
  {
  ...
  ///
  /// \pre <tt>ROW == 2</tt>
  inline Matrix(const Row_vector& v0, const Row_vector& v1)
  {
  ...
  ///
  /// \pre <tt>ROW == 3</tt>
  inline Matrix(const Row_vector& v0, const Row_vector& v1, const Row_vector& v2)
  {
  ...
  /// \pre <tt>ROW == 4</tt>
  inline Matrix(
    const Row_vector& v0, const Row_vector& v1, const Row_vector& v2, const Row_vector& v3)
  {
  ...
  ///
  /// The elements are given in row-major order.
  inline Matrix(T m0, T m1, T m2, T m3, T m4, T m5, T m6, T m7, T m8, T m9, T m10, T m11)
  {
  ...
  ///
  /// The elements are given in row-major order.
  inline Matrix(T m0, T m1, T m2, T m3, T m4, T m5, T m6, T m7, T m8, T m9, T m10, T m11, T m12,
    T m13, T m14, T m15)
  {
  ...
  inline T det33() const
  {
    ...
    return this->xx * this->yy * this->zz + this->xy * this->yz * this->zx +
      this->xz * this->yx * this->zy - this->xx * this->zy * this->yz -
      this->xy * this->zz * this->yx - this->xz * this->zx * this->yy;
  }
  ...
  inline void lookat(const Vector<Float32, 3>& position, const Vector<Float32, 3>& target,
  ...
  /// direction.
  inline void lookat(const Vector<Float64, 3>& position, const Vector<Float64, 3>& target,
...
template <typename T, Size ROW, Size COL>
inline bool operator==(const Matrix<T, ROW, COL>& lhs, const Matrix<T, ROW, COL>& rhs)
{
...
template <typename T, Size ROW, Size COL>
inline Matrix<T, ROW, COL> operator+(const Matrix<T, ROW, COL>& lhs, const Matrix<T, ROW, COL>& rhs)
{
...
template <typename T, Size ROW, Size COL>
inline Matrix<T, ROW, COL> operator-(const Matrix<T, ROW, COL>& mat)
{
...
/// matrices.
template <typename T, Size ROW, Size COL>
inline Matrix<T, ROW, COL>& operator*=(Matrix<T, ROW, COL>& lhs, const Matrix<T, COL, COL>& rhs)
{
...
inline Matrix<T, ROW1, COL2> operator*(
  const Matrix<T, ROW1, COL1>& lhs, const Matrix<T, ROW2, COL2>& rhs)
{
  // There are more efficient ways of implementing this. Its a default solution. Specializations
  // exist below.
...
inline Vector<T, ROW> operator*(const Matrix<T, ROW, COL>& mat, const Vector<T, DIM>& vec)
{
...
inline Vector<T, COL> operator*(const Vector<T, DIM>& vec, const Matrix<T, ROW, COL>& mat)
{
...
inline Matrix<T, NEW_ROW, NEW_COL> sub_matrix(const Matrix<T, ROW, COL>& mat)
{
...
template <typename T, Size ROW, Size COL>
inline Matrix<T, COL, ROW> transpose(const Matrix<T, ROW, COL>& mat)
{
...
inline U transform_point(const Matrix<T, 4, 4>& mat, ///< 4x4 transformation matrix
...
/// matrix \c mat.
template <typename T, typename U>
inline Vector<U, 2> transform_point(const Matrix<T, 4, 4>& mat, ///< 4x4 transformation matrix
  const Vector<U, 2>& point)                                    ///< point to transform
{
  T w = T(mat.xw * point.x + mat.yw * point.y + mat.ww);
  if (w == T(0) || w == T(1))
    return Vector<U, 2>(U(mat.xx * point.x + mat.yx * point.y + mat.wx),
      U(mat.xy * point.x + mat.yy * point.y + mat.wy));
  else
  {
    w = T(1) / w; // optimization
    return Vector<U, 2>(U((mat.xx * point.x + mat.yx * point.y + mat.wx) * w),
...
/// matrix \c mat.
template <typename T, typename U>
inline Vector<U, 3> transform_point(const Matrix<T, 4, 4>& mat, ///< 4x4 transformation matrix
  const Vector<U, 3>& point)                                    ///< point to transform
{
  ...
  if (w == T(0) || w == T(1)) // avoids temporary and division
    return Vector<U, 3>(U(mat.xx * point.x + mat.yx * point.y + mat.zx * point.z + mat.wx),
      U(mat.xy * point.x + mat.yy * point.y + mat.zy * point.z + mat.wy),
      U(mat.xz * point.x + mat.yz * point.y + mat.zz * point.z + mat.wz));
  else
  {
    w = T(1) / w; // optimization
    return Vector<U, 3>(U((mat.xx * point.x + mat.yx * point.y + mat.zx * point.z + mat.wx) * w),
...
/// matrix \c mat.
template <typename T, typename U>
inline Vector<U, 4> transform_point(const Matrix<T, 4, 4>& mat, ///< 4x4 transformation matrix
  const Vector<U, 4>& point)                                    ///< point to transform
{
  return Vector<U, 4>(U(mat.xx * point.x + mat.yx * point.y + mat.zx * point.z + mat.wx * point.w),
...
/// the matrix \c mat.
template <typename T, typename U>
inline U transform_vector(const Matrix<T, 4, 4>& mat, ///< 4x4 transformation matrix
...
/// the matrix \c mat.
template <typename T, typename U>
inline Vector<U, 2> transform_vector(const Matrix<T, 4, 4>& mat, ///< 4x4 transformation matrix
  const Vector<U, 2>& vector)                                    ///< vector to transform
{
  return Vector<U, 2>(
    U(mat.xx * vector.x + mat.yx * vector.y), U(mat.xy * vector.x + mat.yy * vector.y));
}
...
/// the matrix \c mat.
template <typename T, typename U>
inline Vector<U, 3> transform_vector(const Matrix<T, 4, 4>& mat, ///< 4x4 transformation matrix
  const Vector<U, 3>& vector)                                    ///< vector to transform
{
  return Vector<U, 3>(U(mat.xx * vector.x + mat.yx * vector.y + mat.zx * vector.z),
...
{
  return Vector<U, 3>(U(inv_mat.xx * normal.x + inv_mat.xy * normal.y + inv_mat.xz * normal.z),
...
template <typename T, typename U>
inline Vector<U, 3> transform_normal(const Matrix<T, 4, 4>& mat, ///< 4x4 transformation matrix
  const Vector<U, 3>& normal)                                    ///< normal vector to transform
{
  Matrix<T, 3, 3> sub_mat(mat.xx, mat.xy, mat.xz, mat.yx, mat.yy, mat.yz, mat.zx, mat.zy, mat.zz);
  bool inverted = sub_mat.invert();
  if (!inverted)
    return normal;
  return Vector<U, 3>(U(sub_mat.xx * normal.x + sub_mat.xy * normal.y + sub_mat.xz * normal.z),
...
template <typename T, Size ROW, Size COL>
inline void Matrix<T, ROW, COL>::lookat(const Vector<Float32, 3>& position,
  const Vector<Float32, 3>& target, const Vector<Float32, 3>& up)
{
  mi_static_assert(COL == 4 && ROW == 4);
  ...
  Matrix<T, 4, 4> rot(T(xaxis.x), T(yaxis.x), T(zaxis.x), T(0), T(xaxis.y), T(yaxis.y), T(zaxis.y),
    T(0), T(xaxis.z), T(yaxis.z), T(zaxis.z), T(0), T(0), T(0), T(0), T(1));

  // Compute the new position by multiplying the inverse position with the rotation matrix
  Matrix<T, 4, 4> trans(T(1), T(0), T(0), T(0), T(0), T(1), T(0), T(0), T(0), T(0), T(1), T(0),
...
template <typename T, Size ROW, Size COL>
inline void Matrix<T, ROW, COL>::lookat(const Vector<Float64, 3>& position,
  const Vector<Float64, 3>& target, const Vector<Float64, 3>& up)
{
  mi_static_assert(COL == 4 && ROW == 4);
...
public:
  ...
  static bool lu_decomposition(Matrix& lu, // matrix to decompose, modified in place
  ...
  static void lu_backsubstitution(const Matrix& lu, // LU decomposed matrix
...
template <class T, Size DIM>
bool Matrix_inverter<T, DIM, DIM>::lu_decomposition(Matrix& lu, Index_vector& indx)
{
...
void Matrix_inverter<T, DIM, DIM>::lu_backsubstitution(
  const Matrix& lu, const Index_vector& indx, Vector& b)
{
...
template <typename T>
inline Matrix<T, 4, 4>& operator*=(Matrix<T, 4, 4>& lhs, const Matrix<T, 4, 4>& rhs)
{
